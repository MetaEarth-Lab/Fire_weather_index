...
class SRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        topo_lr = F.interpolate(topo.unsqueeze(0), scale_factor=0.5, mode="bilinear").squeeze(0)
        lmask_lr = F.interpolate(lmask.unsqueeze(0), scale_factor=0.5, mode="bilinear").squeeze(0)

        coord = make_coord((sample.shape[-2], sample.shape[-1]))
